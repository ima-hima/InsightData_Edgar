...
def main():
    ...
    with open(output_filename, "w") as output_stream:
        ...
        logwriter = writer(
            output_stream, delimiter=",", quotechar='"', quoting=QUOTE_MINIMAL
        )
        ...
        with open(log_file, newline="") as input_stream:
            ...
            for row in logline:
                ...
                if current_time != prev_time:  # Time has changed
                    ...
                    which_time = (
                        prev_time - interval
                    )  # It may have changed by a number > interval.
                    ...
                    while which_time < current_time - interval:
                        if which_time in ip_lookup:
                            ...
                            for ip, value in sorted(stuff.items(), key=itemgetter(1)):
                                print_row(ip, value, logwriter)
                                del log_table[ip]
                            del ip_lookup[
                                which_time
                            ]  # which_time is expired; remove entire set from ip_lookup.
                        which_time += delta
                    ip_lookup[
                        current_time
                    ] = set()  # Create a new set for the new current_time.
                ...
                if ip in log_table:
                    ...
                    ip_lookup[current_time].add(
                        ip
                    )  # We know there's already a line in the lookup table for current_time.
